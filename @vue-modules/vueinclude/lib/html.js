...
module.exports = function(base_path, file_path) {
    ...
    if (file_path.indexOf(".html.html") >= 0) {
        src_data = HTMLParser.parse(content,ß.vueinclude_html_parser_options).querySelector("body").innerHTML;
        div_class += " preformatted-html";
    } else {
    // this is a html where we will apply striptags
      src_data = striptags(HTMLParser.parse(content,ß.vueinclude_html_parser_options).querySelector("body").innerHTML, ß.vueinclude_striptags_allowed_tags, ß.vueinclude_striptags_tag_replacement);
    }
