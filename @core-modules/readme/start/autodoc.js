...
function extract_doc(file) {
    ...
    for (let c in comment) {
        if (comment[c].value.substring(0, 5).trim() === '@DOC') {
            let me = comment[c].value.substring(5).trim() + '\n';
            d += me;
...
function traverse(path) {
    ...
    let dir = fs.realpathSync(path);
    if (dir !== path) return '';
    if (dir.split('/').pop().indexOf('modules') >= 0) return '';
    if (dir.split('/').pop() === 'static') return '';
    if (dir.split('/').pop() === '.git') return '';
    if (dir.split('/').pop() === 'boilerplate') return '';

    let stat = fs.statSync(dir);
    if (stat.isDirectory()) {
        let content = fs.readdirSync(dir);
        for (let c in content) {
            d += traverse(dir + '/' + content[c]);
        }
    }
    if (stat.isFile()) {
        if (dir.split('.').pop() !== 'js') return '';
        d += extract_doc(dir);
    }
    return d;
}
...
function boilerplate_files(path, dirname) {
    ...
    let dir = fs.realpathSync(path + '/' + dirname);
    let stat = fs.statSync(dir);
    if (stat.isDirectory()) {
        let content = fs.readdirSync(dir);
        for (let c in content) {
...
// autodoc the global boilerplate folder
doc += traverse(ß.BPD + '/global');
...
for (let module in ß.modules) {
    ...
    for (let dir in ß.modules[module]) {
        if (ß.modules[module][dir] === true)
            doc += load_module_doc(module, dir);
    }
